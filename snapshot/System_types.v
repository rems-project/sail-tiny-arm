...
Inductive MemoryOrdering :=
  | Ordering_Relaxed | Ordering_Acquire | Ordering_AcquireRCpc | Ordering_Release.
Definition num_of_MemoryOrdering (arg_ : MemoryOrdering) : Z :=
   match arg_ with
   | Ordering_Relaxed => 0
   | Ordering_Acquire => 1
   | Ordering_AcquireRCpc => 2
   | Ordering_Release => 3
   end.

Definition MemoryOrdering_of_num (arg_ : Z) (*(0 <=? arg_) && (arg_ <=? 3)*) : MemoryOrdering :=
   let l__0 := arg_ in
   if Z.eqb (l__0) (0) then Ordering_Relaxed
   else if Z.eqb (l__0) (1) then Ordering_Acquire
   else if Z.eqb (l__0) (2) then Ordering_AcquireRCpc
   else Ordering_Release.

Lemma MemoryOrdering_num_of_roundtrip (x : MemoryOrdering) : MemoryOrdering_of_num (num_of_MemoryOrdering x) = x.
  destruct x; reflexivity.
Qed.
Lemma num_of_MemoryOrdering_injective (x y : MemoryOrdering) : num_of_MemoryOrdering x = num_of_MemoryOrdering y -> x = y.
  intro.
  rewrite <- (MemoryOrdering_num_of_roundtrip x).
  rewrite <- (MemoryOrdering_num_of_roundtrip y).
  congruence.
Qed.
Definition MemoryOrdering_eq_dec (x y : MemoryOrdering) : {x = y} + {x <> y}.
  refine (match Z.eq_dec (num_of_MemoryOrdering x) (num_of_MemoryOrdering y) with
  | left e => left (num_of_MemoryOrdering_injective x y e)
  | right ne => right _
  end).
  congruence.
Defined.
Definition MemoryOrdering_beq (x y : MemoryOrdering) : bool :=
  Z.eqb (num_of_MemoryOrdering x) (num_of_MemoryOrdering y).
Lemma MemoryOrdering_beq_iff x y : MemoryOrdering_beq x y = true <-> x = y.
  unfold MemoryOrdering_beq.
  rewrite Z.eqb_eq.
  split; [apply num_of_MemoryOrdering_injective | congruence].
Qed.
Lemma MemoryOrdering_beq_refl x : MemoryOrdering_beq x x = true.
apply MemoryOrdering_beq_iff; reflexivity.
Qed.
#[export]
Instance Decidable_eq_MemoryOrdering : EqDecision MemoryOrdering := MemoryOrdering_eq_dec.
#[export]
Instance Countable_MemoryOrdering : Countable MemoryOrdering.
refine {|
  encode x := encode (num_of_MemoryOrdering x);
  decode x := z ← decode x; mret (MemoryOrdering_of_num z);
|}.
abstract (
  intro s; rewrite decode_encode;
  simpl;
  rewrite MemoryOrdering_num_of_roundtrip;
  reflexivity).
Defined.
#[export]
Instance dummy_MemoryOrdering : Inhabited MemoryOrdering := { inhabitant := Ordering_Relaxed }.
...
Inductive operand :=
| OperandReg : reg_index -> operand
| OperandImm : bits 12 -> operand.
Arguments operand : clear implicits.

Definition sail_operand_encode (x : operand) := match x with
  | OperandReg x' => encode (0, encode x')
  | OperandImm x' => encode (1, encode x') end.
Definition sail_operand_decode x : option operand := match decode x with
  | Some (0, x') => OperandReg <$> decode x'
  | Some (1, x') => OperandImm <$> decode x'
  | _ => None end.
...
|}.
#[export]
Instance dummy_operand : Inhabited (operand) := { inhabitant := OperandReg inhabitant }.

Record move_imm_data := {
  move_imm_data_imm : bits 16;
  move_imm_data_hw : bits 2;
}.
Arguments move_imm_data : clear implicits.
#[export]
Instance Decidable_eq_move_imm_data : EqDecision move_imm_data.
   intros [x0 x1].
   intros [y0 y1].
  cmp_record_field x0 y0.
  cmp_record_field x1 y1.
...
Defined.
#[export]
Instance Countable_move_imm_data : Countable move_imm_data.
refine {|
  encode x := encode (move_imm_data_imm x, move_imm_data_hw x);
  decode x := '(x0, x1) ← decode x;
              mret (Build_move_imm_data x0 x1)
|}.
abstract (
  intros [x0 x1];
  rewrite decode_encode;
...
#[export] Instance eta_move_imm_data : Settable _ := settable! Build_move_imm_data <move_imm_data_imm; move_imm_data_hw>.
#[export]
Instance dummy_move_imm_data : Inhabited (move_imm_data) := {
  inhabitant := {| move_imm_data_imm := inhabitant; move_imm_data_hw := inhabitant
|} }.


Inductive move_operand :=
| MoveReg : reg_index -> move_operand
| MoveImm : move_imm_data -> move_operand.
Arguments move_operand : clear implicits.

Definition sail_move_operand_encode (x : move_operand) := match x with
  | MoveReg x' => encode (0, encode x')
  | MoveImm x' => encode (1, encode x') end.
Definition sail_move_operand_decode x : option move_operand := match decode x with
  | Some (0, x') => MoveReg <$> decode x'
  | Some (1, x') => MoveImm <$> decode x'
  | _ => None end.
Lemma sail_move_operand_decode_encode : forall (x : move_operand), sail_move_operand_decode
  (sail_move_operand_encode x)  = Some x.
Proof.
  unfold sail_move_operand_decode, sail_move_operand_encode;
  intros [x|x]; rewrite !decode_encode; reflexivity.
Qed.

#[export]
Instance Decidable_eq_move_operand : EqDecision move_operand := decode_encode_eq_dec
  sail_move_operand_encode sail_move_operand_decode sail_move_operand_decode_encode .

#[export]
Instance Countable_move_operand : Countable move_operand := {|
  encode := sail_move_operand_encode;
  decode := sail_move_operand_decode;
  decode_encode := sail_move_operand_decode_encode
|}.
#[export]
Instance dummy_move_operand : Inhabited (move_operand) := { inhabitant := MoveReg inhabitant }.
...
Inductive ast :=
| Load : (datasize * reg_index * reg_index * operand) -> ast
| Store : (datasize * reg_index * reg_index * operand) -> ast
| LoadAcquire : (datasize * reg_index * reg_index * MemoryOrdering) -> ast
| StoreRelease : (datasize * reg_index * reg_index) -> ast
| BitwiseAnd : (datasize * reg_index * reg_index * operand) -> ast
| ExclusiveOr : (datasize * reg_index * reg_index * reg_index) -> ast
| InclusiveOr : (datasize * reg_index * reg_index * reg_index) -> ast
| Move : (datasize * reg_index * move_operand) -> ast
| Add : (datasize * reg_index * reg_index * operand) -> ast
| Sub : (datasize * reg_index * reg_index * operand) -> ast
| Compare : (datasize * reg_index * operand) -> ast
| ConditionalSelect : (datasize * reg_index * reg_index * reg_index * bits 4) -> ast
| DataMemoryBarrier : (MBReqDomain * MBReqTypes) -> ast
| DataSynchronizationBarrier : (MBReqDomain * MBReqTypes) -> ast
| InstructionSynchronizationBarrier : unit -> ast
| Nop : unit -> ast
| CompareAndBranch : (datasize * reg_index * bits 64 * bool) -> ast
| LogicalShiftRight : (datasize * reg_index * reg_index * operand) -> ast
| Branch : bits 64 -> ast
| TLBInvalidation : (TLBIOp * reg_index) -> ast
| SupervisorCall : bits 16 -> ast
| ExceptionReturn : unit -> ast
| SystemRegisterMove : (bool * bits 3 * bits 4 * bits 4 * bits 3 * reg_index) -> ast.
...
Definition sail_ast_encode (x : ast) := match x with
  ...
  | LoadAcquire x' => encode (2, encode x')
  | StoreRelease x' => encode (3, encode x')
  | BitwiseAnd x' => encode (4, encode x')
  | ExclusiveOr x' => encode (5, encode x')
  | InclusiveOr x' => encode (6, encode x')
  | Move x' => encode (7, encode x')
  | Add x' => encode (8, encode x')
  | Sub x' => encode (9, encode x')
  | Compare x' => encode (10, encode x')
  | ConditionalSelect x' => encode (11, encode x')
  | DataMemoryBarrier x' => encode (12, encode x')
  | DataSynchronizationBarrier x' => encode (13, encode x')
  | InstructionSynchronizationBarrier x' => encode (14, encode x')
  | Nop x' => encode (15, encode x')
  | CompareAndBranch x' => encode (16, encode x')
  | LogicalShiftRight x' => encode (17, encode x')
  | Branch x' => encode (18, encode x')
  | TLBInvalidation x' => encode (19, encode x')
  | SupervisorCall x' => encode (20, encode x')
  | ExceptionReturn x' => encode (21, encode x')
  | SystemRegisterMove x' => encode (22, encode x') end.
Definition sail_ast_decode x : option ast := match decode x with
  | Some (0, x') => Load <$> decode x'
  | Some (1, x') => Store <$> decode x'
  | Some (2, x') => LoadAcquire <$> decode x'
  | Some (3, x') => StoreRelease <$> decode x'
  | Some (4, x') => BitwiseAnd <$> decode x'
  | Some (5, x') => ExclusiveOr <$> decode x'
  | Some (6, x') => InclusiveOr <$> decode x'
  | Some (7, x') => Move <$> decode x'
  | Some (8, x') => Add <$> decode x'
  | Some (9, x') => Sub <$> decode x'
  | Some (10, x') => Compare <$> decode x'
  | Some (11, x') => ConditionalSelect <$> decode x'
  | Some (12, x') => DataMemoryBarrier <$> decode x'
  | Some (13, x') => DataSynchronizationBarrier <$> decode x'
  | Some (14, x') => InstructionSynchronizationBarrier <$> decode x'
  | Some (15, x') => Nop <$> decode x'
  | Some (16, x') => CompareAndBranch <$> decode x'
  | Some (17, x') => LogicalShiftRight <$> decode x'
  | Some (18, x') => Branch <$> decode x'
  | Some (19, x') => TLBInvalidation <$> decode x'
  | Some (20, x') => SupervisorCall <$> decode x'
  | Some (21, x') => ExceptionReturn <$> decode x'
  | Some (22, x') => SystemRegisterMove <$> decode x'
  | _ => None end.
Lemma sail_ast_decode_encode : forall (x : ast), sail_ast_decode (sail_ast_encode x)  = Some x.
Proof.
  unfold sail_ast_decode, sail_ast_encode;
  intros [x|x|x|x|x|x|x|x|x|x|x|x|x|x|x|x|x|x|x|x|x|x|x]; rewrite !decode_encode; reflexivity.
Qed.
...
Definition register_list : list (string * register) := List.concat [
  List.map (fun '(s, r) => (s, R_ProcState r)) register_ProcState_list;
...
Definition PSTATE_ref : register_ref _ :=
  Build_register_ref register type_of_register _ "PSTATE" PSTATE (fun x => x) (fun x => x).
Instance dummy_register_ProcState : Inhabited (register_ref _) := populate PSTATE_ref.
